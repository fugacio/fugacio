...
The thermo equilibrium models -- :class:`~fugacio.thermo.EOSModel` and
:class:`~fugacio.thermo.GammaPhiModel` -- take *array* constants (``tc``, ``pc``,
...
lookup in :mod:`fugacio.sim.properties`) and assembles the activity model from the
...
:class:`~fugacio.thermo.EquilibriumModel`.

The returned object is what the gamma-phi-aware unit operations
(:mod:`fugacio.sim.separations`) and the T-x-y / P-x-y / azeotrope helpers
(:mod:`fugacio.sim.diagrams`) consume, so a flowsheet can switch from
...
def eos_model_for(
    ...
    *,
    ...
    """Build an :class:`~fugacio.thermo.EOSModel` for named ``components``.

    Pass ``use_database_kij=True`` to fill the binary interaction matrix from the
    curated ChemSep Peng-Robinson ``k_ij`` set (:func:`fugacio.thermo.kij_from_database`);
...
def nrtl_model_for(
    ...
    *,
    ...
    ``strict=True``; see :func:`fugacio.thermo.nrtl_from_database`.
    """
    tc, pc, omega, _, _ = _resolve(tuple(components))
...
class UnifacModel:
    """An :class:`~fugacio.thermo.ActivityModel` adapter over predictive UNIFAC.

    Wraps :func:`fugacio.thermo.unifac_activity` (classic, Hansen VLE parameters)
    or :func:`fugacio.thermo.modified_unifac_activity` (Dortmund, T-dependent) so
