...
preparation and retained outputs, with dynamic operating values. Studies evaluate
one forward derivative direction for each design variable, keeping the unit kernels
separate instead of compiling one whole-plant program. Fixed case topology and
dynamic recycle parameters let optimizer trials reuse the compiled maps. Saved
...
The first derivative trace and compilation of a full plant can be lengthy,
particularly with the older JAX release required by Intel Macs. In one local
validation of the 16-stage example, the first optimization derivative took
about 21 minutes; subsequent operating points took 10 to 12 seconds. These
timings depend on hardware, JAX version, and cache state.
...
derivative/optimization, and final audit phases. It retains the disk cache and
all completed artifacts. The depropanizer study requests enable this option.
A 7 GB Linux validation container could solve the baseline but couldn't
complete the full-plant derivative compilation. Clearing caches doesn't
eliminate the peak memory required by each compilation.
